...
console.log(args[0].actorData._id)
...
let wildShapeTransform = async function (actorOriginalForm, actorNewFormId) {
    ...
    //new token ID
    let newTokenId = actorNewForm.data.token._id
    ...
    const applyWildShapeEffect = [{
        ...
        changes: [{
            "key": "macro.execute",
            "mode": 1,
            "value": `"wildshape-effect"` + `"${currentFormActorId}"` + `"${actorOriginalFormImagePath}"` + `"${actorNewFormId}"` + `"${actorNewShapeName}"` + `"${newTokenId}"`,
            "priority": "20"
        }],
        duration: {
            "seconds": math.floor(target.actor.getRollData().classes.druid.levels/2)*3600,
        }
    }]
