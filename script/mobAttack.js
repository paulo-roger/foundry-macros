...
const makeGroup = async () => {
    ...
    const numTokenSpaces = numGroup*token.data.height*token.data.width

    //col then row
    const numCol = Math.ceil(Math.sqrt(numTokenSpaces))
    const numRow = Math.ceil(numTokenSpaces/numCol)

    token.document.update({width:numCol, height:numRow, scale: (token.data.scale*token.data.height)/numCol, 'flags.world.group': {size: numGroup, token: token.document.toObject()}, name: `${token.name} Group (${numGroup})`})
